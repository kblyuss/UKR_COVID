function ukraine_long_forecast
...
R0val=linspace(R0,1.2*R0,14);

for i=1:52 % end of summer
%     uncomment the next three lines and use 1.1*R0 or 1.2*R0 above to plot
%     different scenarios
%     if i<15
%         R0=R0val(i);
%     end;
...
a1=0.6;a2=0.6;a3=0.6;a4=1;
mix_matrix=a1*school+a2*work+a3*other+a4*hhome;
EV2=max(real(eig(mix_matrix)));
    
R0=EV2/EV1*R0;

for i=53:110 % start of academic year
...
axis([0 217 0 2.3e5]);
set(gca,'LineWidth',2,'FontSize',24,'Box','on','xtick',[7 37 68 98 129 160 190],'xticklabel',{'1 Apr','1 May','1 Jun','1 Jul','1 Aug', '1 Sep', '1 Oct'},'ytick',[0 1e5 2e5]);
...
axis([0 217 0 4000])
set(gca,'LineWidth',2,'FontSize',24,'Box','on','xtick',[7 37 68 98 129 160 190],'xticklabel',{'1 Apr','1 May','1 Jun','1 Jul','1 Aug', '1 Sep', '1 Oct'},'ytick',[0 2000 4000]);

NNd=length(ypld);
ypld2=ypld(2:NNd)-ypld(1:(NNd-1));

plot(xpld(1:length(ypld2)),ypld2,'k-','LineWidth',2);%'MarkerSize',8) days3d
...
plot(zpl(1:end),ypld2_d(1:end),'k-','LineWidth',2);

b1=bar(days2,dediffs,'FaceColor',[0.5 0.5 0.5]);
b1.FaceAlpha=0.2;

axis([0 217 0 80]);
set(gca,'LineWidth',2,'FontSize',24,'Box','on','xtick',[7 37 68 98 129 160 190],'xticklabel',{'1 Apr','1 May','1 Jun','1 Jul','1 Aug', '1 Sep', '1 Oct'},'ytick',[0 40 80]);
